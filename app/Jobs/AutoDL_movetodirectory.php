...
class AutoDL_movetodirectory implements ShouldQueue
{
    ...
    {
        $this->onQueue('autodl_movetodirectory');
        $this->delay(1);
    ...
    {   
        /** if a job is dispatched to this queue then it originally came from the AutoDL_movetodirectorychecks queue */
        ...
        if(!$network_connectivity){
            ...
            if($all_case_files->count() != 0){
                ...
                $unzipFolder = storage_path()."/app".config('s3br24.unzip_folder').$caseId;
                ...
                if($count_of_rsync_commands_running > 5){
                    ...
                    \App\Jobs\AutoDL_movetodirectory::dispatch($caseId)->delay(now()->addSeconds($queue_delay_seconds_autodl));
                    return;
                }
                ...
                $rsync_progress_log = storage_path()."/logs".config('s3br24.download_log')."progress/". $caseId.'_rsync_progressLog.log';
                ...
                foreach($all_case_files as $case_file_key => $case_downloadfile_details){
                    ...
                    dump($case_downloadfile_details->id);
                    ...
                    if($case_file_key == $last_key){
                        ...
                        dump('job dispatched to AutoDL_messageaftermovetodirectory queue');
                    }
                }
            }else{
    ...
    public function failed(Exception $e)
    {
        ...
        Loggy::write('default', json_encode([
            'success' => false,
            'description' => 'AutoDL_movetodirectory failed()',
            'exception' => $e,
            'caseId' => $caseId,
            //'caseIdtype' => $caseIdtype
        ]));
