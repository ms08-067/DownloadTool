...
(function($) {
    ...
    function Plugin($container, options) {
         ...
         */
        function _setImage() {
            var colorPicker = new Image(), backgroundUrl = $track.css("background-image").replace(/"/g, "").replace(/url\(|\)$/ig, "");
            ...
            if(hasCanvas) {
                $color.bind(eventType, function(event) {
                    if ($(".track").is(":visible") == true) {
                        ...
                        /** and make the clicked color index style="z-index: 13;*/
                        $('.color').css('z-index', '');
                        $color.css('z-index', '13');
                    }else{
                        /** make all reset z-index */
                        $('.color').css('z-index', '');
                        /** assign this the correct z-index */
                        $color.css('z-index', '13');
                    }
                    ...
                    $(document).bind("mousedown.colorpicker", function(event) {
                        ...
                        /** revert the z-index back to none */
                        $color.css('z-index', '');
