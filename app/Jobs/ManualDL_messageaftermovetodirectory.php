...
use Loggy;
use File;
use App\Events\NewManualDLJobData;
use Debugbar;
...
class ManualDL_messageaftermovetodirectory implements ShouldQueue
{
    ...
    {
        $this->onQueue('manualdl_messageaftermovetodirectory');
        $this->delay(1);
    ...
    {
        /** if a job is dispatched to this queue then it originally came from the ManualDL_messageaftermovetodirectorychecks queue */
        ...
        if(!$network_connectivity){
            ...
            $all_case_files = TaskManualDownloadFile::where('case_id', $caseId)->where('state', '=', 'moving_to_jobFolder')->where('state', '!=', 'notified')->get();
            ...
            foreach($all_case_files as $case_file_key => $case_downloadfile_details){
                ...
                /** they will never be notified they were all just set to moving.. */
                ...
                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE = env('MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE');
                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE = str_replace("[XxFOLDERNUMBERxX]", $file->case_id, $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE);
                if($case_downloadfile_details->xml_tool_client == 1){
                    /**Germany*/
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE = str_replace("[XxFOLDERNAMExX]", "Germany", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE);
                }else if($case_downloadfile_details->xml_tool_client == 2){
                    /**Asia*/
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE = str_replace("[XxFOLDERNAMExX]", "Asia", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE);
                }else{
                    /**Default*/
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE = str_replace("[XxFOLDERNAMExX]", "Other", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE);
                }


                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING = env('MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING');
                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED = env('MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED');

                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING = str_replace("[XxFOLDERNUMBERxX]", $file->case_id, $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING);
                $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED = str_replace("[XxFOLDERNUMBERxX]", $file->case_id, $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED);
                if($case_downloadfile_details->xml_tool_client == 1){
                    /**Germany*/
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING = str_replace("[XxFOLDERNAMExX]", "Other", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING);
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED = str_replace("[XxFOLDERNAMExX]", "Other", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED);
                }else if($case_downloadfile_details->xml_tool_client == 2){
                    /**Asia*/
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING = str_replace("[XxFOLDERNAMExX]", "Other", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING);
                    $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED = str_replace("[XxFOLDERNAMExX]", "Other", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED);
                ...
                $formatUrl = str_replace("/", "\\", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE);
                ...
                $attachment = $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING . '||' . $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED . '||' . $linkUrl . '||' . $formatUrl;

                //$content .= 'Zip '.$file->type.' extracted to ' . $folder . '';
                if($content == ''){
                    //$blankImage = '[img width="0" height="0" alt="" title=""][/img]';
                    // $content .= '[BR]------------------------------------------------------[BR]';
                    // $content .= '[URL='.$MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING.']'. $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED.'[/URL]';
                    // $content .= '[BR]------------------------------------------------------[BR]';
                    // $content .= '<a href="'.$MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING.'" target="_blank">'.$MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_DECODED.'</a>';
                    // $content .= '[BR]------------------------------------------------------[BR]';

                    // $content .= '[BR]------------------------------------------------------[BR]';
                    // $content .= '[URL=file:\\\\'.str_replace("/", "\\", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE . $file->case_id).']'. str_replace("/", "\\", $MANUAL_JOBFOLDER_DIR_SHARELOCATION_STRING_ALTERNATE . $file->case_id).'[/URL]';
                    // $content .= '[BR]------------------------------------------------------[BR]';
            ...
            if($result_of_folder_move == 'true'){
                ...
                try {
                    DB::beginTransaction();
                    $task_manual_download = TaskManualDownload::where('case_id', $case_id)->first();
                    if($task_manual_download){
                        $data = [
                            'case_id' => $case_id,
                            'state' => 'downloaded',
                            'try' => 0,
                    ...
                    foreach($all_case_files as $case_file_key => $case_downloadfile_details){
                        ...
                        $file->state = 'notified';
                        $file->save();
                        dump('set type to notified');
                    }

                    //event(new NewManualDLJobData('New Manual DL Job Id = '.$case_id));
                    //dump('NewManualDLJobData websocket event triggered 23AUG2023 pusher was disabled');
                } catch(\Exception $e) {
                    Debugbar::addException($e);
                    throw $e;
                }
            }else{
                dump('checking the pid is still running handling for both scenarios');
                ...
                foreach($all_case_files as $case_file_key => $case_downloadfile_details){
                    ...
                    if($check_if_pid_still_running == $file['pid']){
                        ...
                        //\App\Jobs\ManualDL_unzip::dispatch($file->case_id, $file->type)->delay(now()->addSeconds($queue_delay_seconds_manualdl));
    ...
    public function physically_move_caseID_folder_from_unzip_directory_to_jobfolder_directory($case_id)
    {
        ...
        $all_case_files = TaskManualDownloadFile::where('case_id', $case_id)->where('state', '!=', 'notified')->get();
    ...
    public function failed(Exception $e)
    {
        ...
        Loggy::write('default', json_encode([
            'success' => false,
            'description' => 'ManualDL_messageaftermovetodirectory failed()',
            'exception' => $e,
            'caseId' => $caseId
        ]));
    } 
