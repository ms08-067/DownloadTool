...
class ManualDL_unzipchecks implements ShouldQueue
{
    ...
    {
        $this->onQueue('manualdl_unzipchecks');
        $this->delay(1);
    ...
    public function handle()
    {
        ...
        if(!$network_connectivity){
            ...
            $file = TaskManualDownloadFile::select('*')
            ...
            if(empty($file) || $file == null) {
                ...
                Loggy::write('default', json_encode([
                    'success' => false,
                    'description' => 'function ManualDL_unzipchecks handle() could not find row on taskManualDownloadFile table using query parameters',
                    'caseId' => $caseId,
                    'caseIdtype' => $caseIdtype,
            ...
            if ($tryOpeningZip !== TRUE) {
                ...
                if ($unzip_tries == config('s3br24.manual_unzip_retry_count')) {
                    ...
                    if($messenger_destination == 'BITRIX'){
                        ...
                        \App\Jobs\ManualDL_unzipchecks::dispatch($file->case_id, $file->type)->delay(now()->addSeconds($queue_delay_seconds_manualdl));
                        return;
                    }
                }
    ...
    public function failed(Exception $e)
    {
        ...
        Loggy::write('default', json_encode([
            'success' => false,
            'description' => 'ManualDL_unzipchecks failed()',
            'exception' => $e,
            'caseId' => $caseId,
            'caseIdtype' => $caseIdtype
        ]));
