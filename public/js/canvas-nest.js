! function() {
    ...
    function l() {
        var i = j("script"),
            w = i.length,
            v = i[w - 1];
        return { l: w, z: o(v, "zIndex", 0), o: o(v, "opacity", 0.6), c: o(v, "color", "18,7,84"), n: o(v, "count", 0) }
    }
        ...
        t.forEach(function(i) {
            ...
            for (v = 0; v < w.length; v++) {
                ...
                if (i !== x && null !== x.x && null !== x.y) {
                    ...
                    y < x.max && (x === f && y >= x.max / 2 && (i.x -= 0.03 * B, i.y -= 0.03 * z), A = (x.max - y) / x.max, e.beginPath(), e.lineWidth = A / 2, e.strokeStyle = "rgba(" + s.c + "," + (A + 0.1) + ")", e.moveTo(i.x, i.y), e.lineTo(x.x, x.y), e.stroke())
                }
            }
