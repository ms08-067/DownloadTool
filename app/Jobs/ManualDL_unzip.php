...
class ManualDL_unzip implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if(!$network_connectivity){
            ...
            $file = TaskManualDownloadFile::select('*')->where('state', 'downloaded')->where('unzip', 1)->where('unzip_checks', 1)
            ...
            if(empty($file) || $file == null) {
                ...
                Loggy::write('default', json_encode([
                    'success' => false,
                    'description' => 'function ManualDL_unzip handle() could not find row on taskManualDownloadFile table using query parameters',
                    'caseId' => $caseId,
                    'caseIdtype' => $caseIdtype
                ]));
                return;
            ...
            foreach($all_case_files as $case_file_key => $case_downloadfile_details){
                ...
                $file = TaskManualDownloadFile::select('*')->where('id', $case_downloadfile_details->id)->first();
                ...
                if ($unzip_tries >= config('s3br24.manual_unzip_retry_count')) {
                    ...
                    Loggy::write('default', json_encode([
                        'success' => false,
                        'description' => '$unzip_tries > config("s3br24.manual_unzip_retry_count")',
                        'caseId' => $caseId,
                        'caseIdtype' => $caseIdtype
                    ]));
                    ...
                    \App\Jobs\ManualDL_download::dispatch($file->case_id, $file->type)->delay(now()->addSeconds($queue_delay_seconds_manualdl));
                    return;
                }
                ...
                $zipArchive_numFiles = $zipArchive->numFiles + 1; /** the three extra lines */
                if($file->file_count == null){
                    $file->file_count = $zipArchive_numFiles;
                    $file->save();
                }
                ...
                /** just a note that this will actually always run true because the zip test does not clear the lob from the time it did the test. so it will find the test results */
    ...
    public function failed(Exception $e)
    {
        ...
        Loggy::write('default', json_encode([
            'success' => false,
            'description' => 'ManualDL_unzip failed()',
            'exception' => $e,
            'caseId' => $caseId,
            'caseIdtype' => $caseIdtype
        ]));
