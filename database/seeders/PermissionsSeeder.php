...
class PermissionsSeeder extends Seeder
{
    ...
    public function run()
    {
    	...
    	foreach ($roles as $role) {
    		$rolesArray[$role] = Group::create(['name' => $role]); /** for migrate:fresh instances */
    		//$rolesArray[$role] = Group::select('*')->where(['name' => $role])->get()->toArray(); /** for production */
    	}
    	...
    	foreach ($permissions as $permission => $authorized_roles) {
    		...
    		foreach ($authorized_roles as $role) {
    			//$fk_role_id = $rolesArray[$role][0]['id']; /** for Group::select */
    			$fk_role_id = $rolesArray[$role]->id; /** for Group::create */
