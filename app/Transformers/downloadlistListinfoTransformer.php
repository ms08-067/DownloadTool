...
/**
 * Class downloadlistListinfoTransformer
 *
 * @author sigmoswitch
 * @package App\Transformers
 */
class downloadlistListinfoTransformer extends TransformerAbstract
{
    /**
    ...
    public function transform($resource)
    {
        ...
        /**dump($search_query_type_highlighting);*/
        ...
        if (is_numeric($search_query_type_highlighting)) {
            ...
            /** we can alter the onclick to remove the styling before the multiselect */
        ...
        /**dump(gettype($resource->xml_tool_client));*/
        /**dump(is_null($resource->xml_tool_client));*/
        /**dd($resource->xml_tool_client);*/

        $tool_client = '';
        if($resource->xml_tool_client == 'null'){
            $tool_client = '';
        }else if($resource->xml_tool_client == 1){
            $tool_client = 'DE';
        }else if($resource->xml_tool_client == 2){
            $tool_client = 'VN';
        }else{
            $tool_client = '';
        }
        ...
        /** now we RE-ENABLE the ability to change the colors manually so we can safely ignore the custom_color column from the db */
        /** so we move it all after the other colors to take precedence */
        /**dd($resource->custom_color);*/
        if($resource->custom_color == null){
            $custom_color_rgb_manually_chosen = '#FFFFFF';
        }else{
            $rgb = $this->hex2rgb($resource->custom_color);
            $custom_color_rgb_manually_chosen = 'rgb('.$rgb['red'].', '.$rgb['green'].', '.$rgb['blue'].')';
            /**dump($custom_color_rgb_manually_chosen);*/
        }
       ...
       $array_of_cases_to_override_today_row_color = ['downloaded', 'in progress', 'check'];
        ...
        return [
            ...
            'xml_jobid_title' => $resource->xml_jobid_title,
            ...
            'xml_jobinfo' => $resource->xml_jobinfo,
            ...
            'custom_color_rgb_manually_chosen' => $custom_color_rgb_manually_chosen,
