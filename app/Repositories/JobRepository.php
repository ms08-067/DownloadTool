...
class JobRepository extends Repository
{
    ...
    public function reDownloadWhenServerRestart()
    {
        ...
        /**dump($date);*/
        $filesDownloading = $this->taskDownloadFile
        ...
        foreach ($filesDownloading as $file_part_of_case) {
            ...
            $all_case_files = $this->taskDownloadFile->where('case_id', $file_part_of_case->case_id)->get();
            foreach($all_case_files as $case_file_key => $case_downloadfile_details){

                $file = $this->taskDownloadFile->where('id', $case_downloadfile_details->id)->first();
    ...
    public function download()
    {
        ...
        $count = $this->taskDownloadFile->where('state', 'downloading')->count();
        /**dump($count);*/
        if ($count >= 10) {
            $limit = 0;
        } else {
        ...
        $filesDownload = $this->taskDownloadFile->where('state', 'new')->offset(0)->limit($limit)->get();
        /**dump($filesDownload);*/
        foreach ($filesDownload as $file) {
            if ($file['from'] == $br24Config['from_s3']) {
                $cmd="aria2c --allow-overwrite=true --piece-length=100M --max-connection-per-server=8 --file-allocation=none --max-concurrent-downloads=8 --split=8 --log-level=notice --max-tries=12  --retry-wait=15 --log={$log}  --dir={$dir} " . '"' . $file['url'] . '"';
        ...
        $filesDownloading = $this->taskDownloadFile->where('state', 'downloading')->get();
        /**dd($filesDownloading);*/
        $date = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . "-30 minutes"));
    ...
    public function unZipChecks()
    {
        ...
        $file = $this->taskDownloadFile->where('state', 'downloaded')->where('unzip', 0)->first();
    ...
    public function actually_unzip()
    {
        ...
        $file = $this->taskDownloadFile->where('state', 'downloaded')->where('unzip', 1)->where('unzip_checks', 1)->orderBy('updated_at', 'asc')->get();
        ...
        foreach($file as $file_key => $download_task_file_details){
            ...
            if($remembering_caseId_doing != $download_task_file_details->case_id){
                ...
                $all_case_files = $this->taskDownloadFile->where('case_id', $download_task_file_details->case_id)->get();
                /**dd($all_case_files);*/
            ...
            if($try_to_download_again == true || $count_CRC_error > 0){
                ...
                if(exec('grep ' . escapeshellarg($searchString) . ' ' . $unzip_log)) {
                    ...
                    if($file->type == 'example'){
                        ...
                        dd('encountered a type we did not expect');
                    }
                    dump($cmd);
                    exec($cmd);
                }
    ...
    {
        $file = $this->taskDownloadFile->where('state', 'unzipping')->where('unzip', 1)->first();
    ...
    {
        $file = $this->taskDownloadFile->where('state', 'unzipped')->where('unzip', 2)->get();
        /**dump($file);*/
    ...
    public function start_physically_move_caseID_folder_from_unzip_directory_to_jobfolder_directory_using_rsync($case_id)
    {
        ...
        $all_case_files = $this->taskDownloadFile->where('case_id', $case_id)->get();
        ...
        $cmd = 'rsync --ignore-existing -ar '.$unzipFolder. ' ' . $jobFolder;
        $pid = exec($cmd . " > /dev/null & echo $!;", $output);
    ...
    public function send_message_when_case_is_fully_extracted_move_to_directory_of_choice()
    {
        ...
        $file = $this->taskDownloadFile->where('state', 'moving_to_jobFolder')->where('unzip', 2)->orderBy('updated_at', 'asc')->get();
        ...
        foreach($all_case_files as $case_file_key => $case_downloadfile_details){
            ...
            if($content == ''){
                $content .= '```';
                $content .= str_replace("/", "\\", $JOBFOLDER_DIR_SHARELOCATION_STRING . $file->case_id);
                $content .= '```';
        ...
        if(!$one_already_notified && $result_of_folder_move){
            ...
            try {
                DB::beginTransaction();
                $task_download = $this->taskDownload->where('case_id', $case_id)->first();
                if($task_download){
                    $data = [
                        'case_id' => $case_id,
                        'state' => 'downloaded',
    ...
    public function physically_move_caseID_folder_from_unzip_directory_to_jobfolder_directory($case_id)
    {
        ...
        if($check_find_file_count_command_is_still_running_jobfolder_directory >= 2 || $check_find_directory_count_command_is_still_running_jobfolder_directory >= 2 || $check_find_command_is_still_running_unzipfolder_directory >= 2 || $check_find_directory_count_command_is_still_running_unzipfolder_directory >= 2){
            ...
            $number_of_files_in_jobfolder_directory = exec("find ".$inner_jobfolder." -type f | wc -l");
            $number_of_folders_in_jobfolder_directory_min_depth = exec("find ".$inner_jobfolder." -mindepth 1 -type d | wc -l");
            dump('number_of_files_in_jobfolder_directory '.$inner_jobfolder. ' => ' . $number_of_files_in_jobfolder_directory);
            dump('number_of_folders_in_jobfolder_directory_min_depth '.$inner_jobfolder. ' => ' . $number_of_folders_in_jobfolder_directory_min_depth);

            $number_of_files_in_unzipfolder_directory = exec("find ".$inner_unzipfolder." -type f | wc -l");
            $number_of_folders_in_unzipfolder_directory_min_depth = exec("find ".$inner_unzipfolder." -mindepth 1 -type d | wc -l");
            dump('number_of_files_in_unzipfolder_directory '.$inner_unzipfolder. ' => ' . $number_of_files_in_unzipfolder_directory);
            dump('number_of_folders_in_unzipfolder_directory_min_depth '.$inner_unzipfolder. ' => ' . $number_of_folders_in_unzipfolder_directory_min_depth);
            ...
            if($counts_of_items_in_jobfolder == $counts_of_items_in_unzipfolder){
                return true;
            }else{
    ...
    public function after_zip_move_from_temp_upload_directory_to_jobfolder()
    {
        ...
        $file = $this->taskUpload->where('state', '=', 'zipped')->where('state', '!=', 'notified')->where('move_to_jobfolder', 0)->orderBy('updated_at', 'asc')->get();
        ...
        foreach($file as $file_key => $upload_task_file_details){
            ...
            if($remembering_caseId_doing != $upload_task_file_details->case_id){
                ...
                $all_case_files = $this->taskUpload->where('case_id', $upload_task_file_details->case_id)->get();
        ...
        foreach($all_case_files as $case_file_key => $case_uploadfile_details){
            $file = $this->taskUpload->where('id', $case_uploadfile_details->id)->first();
    ...
    public function send_message_when_uploaded_case_is_fully_extracted_move_to_jobfolder_ready_directory()
    {
        ...
        /** and when a job gets notified if there are backlogs of tasks then it doesn't complete correctly */
        /** in fact it actually deletes the jobFolder contetns wasting time and effort */
        $file = $this->taskUpload->where('move_to_jobfolder', 2)->orderBy('updated_at', 'asc')->get();
        ...
        foreach($file as $file_key => $download_task_file_details){
            ...
            if($remembering_caseId_doing != $download_task_file_details->case_id){
                ...
                $all_case_files = $this->taskUpload->where('case_id', $download_task_file_details->case_id)->get();
        ...
        foreach($all_case_files as $case_file_key => $case_downloadfile_details){
            ...
            $file = $this->taskUploadView->where('id', $case_downloadfile_details->id)->first();
            /**dump('processing === ' . $file->local);*/
            /**dump($file->local);*/
            ...
            if($content == ''){
                $content .= '```';
                $content .= str_replace("/", "\\", $JOBFOLDER_DIR_SHARELOCATION_STRING . $file->case_id).'\\ready';
                $content .= '```';
        ...
        if($result_of_folder_move){
            ...
            $file = $this->taskUpload->where('case_id', $remembering_caseId_doing)->first();
            /**dump('processing === ' . $file->local);*/
            /**dump($file->local);*/
    ...
    public function send_the_ready_zip_to_s3()
    {
        ...
        /** and when a job gets notified if there are backlogs of tasks then it doesn't complete correctly */
        /** in fact it actually deletes the jobFolder contetns wasting time and effort */
        $file = $this->taskUpload->where('sending_to_s3', 1)->orderBy('updated_at', 'asc')->get();
    ...
    public function check_the_progress_of_ready_zip_to_s3()
    {
        ...
        /** and when a job gets notified if there are backlogs of tasks then it doesn't complete correctly */
        /** in fact it actually deletes the jobFolder contetns wasting time and effort */
        $file = $this->taskUpload->where('sending_to_s3', 2)->where('state', '=', 'checking upload s3')->orderBy('updated_at', 'asc')->get();
        ...
        if($check_if_pid_still_running == $file->pid){
            ...
            if(!empty($difference)){
                ...
                $file = $this->taskUploadView->where('case_id', $remembering_caseId_doing)->first();
                $JOBFOLDER_DIR_SHARELOCATION_STRING = env('JOBFOLDER_DIR_SHARELOCATION_STRING');
    ...
    public function archive_old_jobs()
    {
        ...
        $dir  = storage_path()."/app".config('s3br24.archive_folder');

        if (!File::isFile($dir)) {
            $archivepath = storage_path().'/app'.config('s3br24.archive_folder');
        ...
        $min_date = Carbon::now()->subMonths(2)->firstOfMonth()->format('Y-m-d');
        /**dd($min_date);*/

        $downloadlist_list_view = DB::table('v_download_files')
        ->whereNotIn('archived_case', [2])
        ->when($min_date, function($query) use ($min_date){
            return $query->whereRaw("DATE(expected_delivery_date_coalesce) <= DATE('$min_date')");
        })
        ->orderByRaw("
            CASE status_of_case
        ...
        foreach($downloadlist_list_view as $case_id_key => $download_details){

            /** we check if the folder exists in the jobfolder directory */
            $jobpath = storage_path().'/app'.config('s3br24.job_folder').$case_id_key;
            ...
            if (File::exists($jobpath)) {
                ...
                if($number_of_files_in_jobs_directory == 0){
                    ...
                    $file = $this->taskDownload->where('id', $download_details->id)->first();
                    /**dd($file);*/
    ...
    public function make_database_file_backup()
    {
        ...
        $dir = storage_path()."/app".config('s3br24.archive_folder').'DB_BACKUP';

        if (!File::isFile($dir)) {
            $path = storage_path().'/app'.config('s3br24.archive_folder').'DB_BACKUP';
            File::makeDirectory($path, 0777, true, true); /** make directory */
        }
    ...
    {
        $td = $this->taskDownload->where('state', 'new')->get();
        foreach ($td as $t) {
            $caseId = $t->case_id;

            $totalFolderJobUnzip = $this->taskDownloadFile
            ->where('case_id', $caseId)
            ->where('state', 'downloaded')
            ->where('unzip', 2)
            ->count();

            $totalFolderJob = $this->taskDownloadFile
            ->where('case_id', $caseId)
            ->count();
